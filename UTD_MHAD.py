...
def encoder2(x, h_dim, z_dim,reuse=False):
    with tf.variable_scope('encoder', reuse=reuse):#reuse非常有用，可以避免设置
        ...
        block_1_in=tf.layers.conv2d(x, h_dim, kernel_size=[3, 3], dilation_rate=[1, 2],padding='SAME')
        block_1_in = tf.nn.relu(block_1_in)

        block_1_out = tf.layers.conv2d(block_1_in, h_dim, kernel_size=[3, 3], dilation_rate=[1, 2],padding='SAME')  # 64 filters, each filter will generate a feature map.
        ...
        block_2_out = tf.layers.conv2d(block_2_in, h_dim*2, kernel_size=[3, 3], dilation_rate=[1, 2],padding='SAME')
        ...
        block_3_out = tf.layers.conv2d(block_3_in, h_dim*3, kernel_size=[3, 3], dilation_rate=[1, 2],padding='SAME')
