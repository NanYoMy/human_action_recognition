...
def conv_block(inputs, out_channels, name='conv'):
    with tf.variable_scope(name):
        conv = tf.layers.conv2d(inputs, out_channels, kernel_size=3, padding='SAME')#64 filters, each filter will generate a feature map.
...
for i, tc in enumerate(train_classes):   #enumerate=> i,data
    alphabet, character, rotation = tc.split('/')
    rotation = float(rotation[3:])
    im_dir = os.path.join(root_dir, 'data', alphabet, character)
    im_files = sorted(glob.glob(os.path.join(im_dir, '*.png')))#glob
...
print(train_dataset.shape)#(4112, 20, 28, 28)
...
for ep in range(n_epochs):
    for epi in range(n_episodes):
        ...
        support = np.zeros([n_way, n_shot, im_height, im_width], dtype=np.float32)#n_shot表示样本的数目
